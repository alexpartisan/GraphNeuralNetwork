...
ACSF_GENERATOR = ACSF(
    ...
    rcut = 10.0,
    g2_params= [[1, 2], [1, 6]],
    g4_params=[[1, 4, 1], [0.1, 4, 1], [1, 4, -1], [0.1, 4, -1]]
)
...
class GraphConv(nn.Module):
    def __init__(self, node_dim, edge_dim):
        ...
        self.encoder = nn.Sequential(
            LinearBn(edge_dim, HIDDEN_DIM*2),
            nn.ReLU(inplace=True),
            LinearBn(HIDDEN_DIM*2, HIDDEN_DIM*2),
            nn.ReLU(inplace=True),
            LinearBn(HIDDEN_DIM*2, HIDDEN_DIM),
            nn.ReLU(inplace=True),
            LinearBn(HIDDEN_DIM, node_dim * node_dim),
            #nn.ReLU(inplace=True),
        )
    ...
    def forward(self, node, edge_index, edge, hidden):
        ...
        # in this case, (edge_num*1*node_dim) @ (edge_num*node_dim*node_dim) -> (edge_num*1*node_dim)
        message = x_i.view(-1,1,node_dim)@edge  # @ operator as matrix-vector and matrix-matrix multiplication
        message = message.view(-1,node_dim) # (edge_num*node_dim)
        # explanation  https://pytorch-scatter.readthedocs.io/en/latest/functions/mean.html
        # source: message(edge_num*node_dim) index: (1*edge_num) out: (num_node*node_dim)
        message = scatter_('mean', message, edge_index[1], dim_size=num_node)
        message = F.relu(message +self.bias)

        #2. update: n_j = f(n_j, m_j)
        update = message # (num_node*node_dim)

        # input:1*num_node*node_dim   hidden:1*num_node*HIDDEN_DIM
        # Required:
        # input of shape (seq_len, batch, input_size), h_0 of shape (num_layers * num_directions, batch, hidden_size)
...
class Set2Set(torch.nn.Module):
    ...
    def __init__(self, in_channel, processing_step=1):
        ...
        # self.lstm = torch.nn.GRU(out_channel, in_channel, batch_first=False, bidirectional=False)
...
class Net(torch.nn.Module):
    ...
    # def __init__(self, node_dim=NODE_DIM, edge_dim=EDGE_DIM, num_target=NUM_TARGET):
        ...
        self.num_propagate = 3 # To reduce complexity, drop from initial 6 to 3
        self.num_s2s = 6 # To reduce complexity, drop from initial 6 to 3 to 0


        # node_dim -> 128
        self.preprocess = nn.Sequential(
            LinearBn(node_dim, HIDDEN_DIM),
            nn.ReLU(inplace=True),
            LinearBn(HIDDEN_DIM, HIDDEN_DIM),
            nn.ReLU(inplace=True),
        )

        self.propagate = GraphConv(HIDDEN_DIM, edge_dim)
        self.set2set = Set2Set(HIDDEN_DIM, processing_step=self.num_s2s)


        #predict coupling constant
        self.predict = nn.Sequential(
            LinearBn(4*HIDDEN_DIM, 8*HIDDEN_DIM),  #node_hidden_dim
            nn.ReLU(inplace=True),
            LinearBn(8*HIDDEN_DIM, 4*HIDDEN_DIM),
            nn.ReLU(inplace=True),
            nn.Linear(4*HIDDEN_DIM, num_target),
        )
...
def run_train(out_dir, trn_split, val_split, num_iters, lr=0.001, initial_checkpoint=None):


    # Adjust learning rate
    # schduler = NullScheduler(lr=0.001)
    schduler = NullScheduler(lr=lr)
    ...
    rate       = 0
    if initial_checkpoint is not None:
        initial_optimizer = initial_checkpoint.replace('model.pth','optimizer.pth')
        ...
        for node, edge, edge_index, node_index, coupling_value, coupling_index, infor in train_loader:
            ...
            #while 1:
                ...
                if iter in iter_save:
                    ...
                    torch.save(net.state_dict(),out_dir +'/checkpoint/model.pth')
                    # torch.save({
                    ...
                    torch.save({
                        ...
                        'iter'     : iter,
                        'epoch'    : epoch,
                    }, out_dir +'/checkpoint/optimizer.pth')
                    pass
...
if __name__ == '__main__':
    ...
    # batch_size 24, from scratch
    # run_train(out_dir=RES_DIR+'/0709_2', lr=0.001, initial_checkpoint=None)

    # prediction
    # model = \
    #     '/home/alexpartisan/Work/Data/Competition/CHAMP/GNN/results/0709_2/checkpoint/00040000_model.pth'
    # run_submit(model, out_dir=RESULT_DIR)


    ###################################################################################
    ## 0710 1e-3 -> 1e-4   Saved for 3-Fold blending
    ###################################################################################
    # batch_size 16, from scratch
    # res_dir = RES_DIR+'/0710_1'
    # trn_split = 'train_split_by_mol_0_56668.npy'
    # val_split = 'valid_split_by_mol_0_28335.npy'
    #
    # run_train(out_dir=res_dir,
    #           trn_split=trn_split,
    #           val_split=val_split,
    #           lr=0.001,
    #           initial_checkpoint=None)
    #
    # model = res_dir+'/checkpoint/model.pth'
    # run_train(out_dir=res_dir,
    #           trn_split=trn_split,
    #           val_split=val_split,
    #           lr=0.0001,
    ...
    #
    #
    # res_dir = RES_DIR+'/0710_2'
    # trn_split = 'train_split_by_mol_1_56669.npy'
    # val_split = 'valid_split_by_mol_1_28334.npy'
    #
    # run_train(out_dir=res_dir,
    #           trn_split=trn_split,
    #           val_split=val_split,
    ...
    #
    #
    # res_dir = RES_DIR+'/0710_3'
    # trn_split = 'train_split_by_mol_2_56669.npy'
    # val_split = 'valid_split_by_mol_2_28334.npy'
    #
    # run_train(out_dir=res_dir,
    #           trn_split=trn_split,
    #           val_split=val_split,
    ...
    ###################################################################################
    ## 0710 Feature Selection based on 1 epoch
    ###################################################################################
    # Boris ACSF params
    # 7/10/15:45
    # run_train(out_dir=res_dir,
    ...
    # Prediction by model trained with 40.9 epoch
    # run_submit(model, out_dir=RESULT_DIR)


    res_dir = RES_DIR+'/0711_2'
    trn_split = 'train_split_by_mol_1_56669.npy'
    val_split = 'valid_split_by_mol_1_28334.npy'
